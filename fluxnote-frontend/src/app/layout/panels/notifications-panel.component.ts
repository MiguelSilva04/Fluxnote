      ...
      <aside class="fixed right-0 top-0 h-full w-full sm:w-96 bg-white dark:bg-gray-800 shadow-2xl z-50 flex flex-col transition-transform duration-300 ease-in-out"
        ...
        <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ unreadCount }} unread notifications</p>
          </div>
        ...
        <div class="flex-1 overflow-y-auto">
          @for (notification of notifications; track notification.id) {
            <div
              [class]="'px-6 py-4 border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer ' + (notification.unread ? 'bg-blue-50/50 dark:bg-blue-900/20' : '')"
            >
              <div class="flex gap-3">
                <div [class]="'p-2 rounded-lg shrink-0 ' + (notification.unread ? 'bg-[#155347]' : 'bg-gray-100 dark:bg-gray-700')">
                  <lucide-icon
                    [name]="notification.icon"
                    [class]="'h-5 w-5 ' + (notification.unread ? 'text-white' : 'text-gray-500 dark:text-gray-400')"
                  ></lucide-icon>
                </div>
