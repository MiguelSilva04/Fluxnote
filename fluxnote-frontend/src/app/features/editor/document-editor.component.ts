  ...
  template: `
    <div class="min-h-screen bg-gray-50 flex flex-col relative">
      ...
      @if (isLoading()) {
        ...
        <!-- Header -->
        <header class="bg-white border-b border-gray-200 px-3 md:px-6 py-2 md:py-4 flex items-center justify-between gap-2 shrink-0">
          <div class="flex items-center gap-2 md:gap-4 min-w-0 flex-1">
            <button (click)="navigateBack()" class="p-2 hover:bg-gray-100 rounded-lg transition-colors flex-shrink-0">
          ...
          @if (canEdit()) {
            ...
            <app-button variant="outline" size="sm" [leftIcon]="true" (onClick)="showWipModal.set(true)" customClass="hidden md:inline-flex">
