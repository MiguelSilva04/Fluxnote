...
@Component({
  ...
  template: `
    <section id="precos" class="py-20 bg-gray-50 dark:bg-gray-900">
          ...
          @for (plan of plans; track plan.nameKey) {
            ...
            >
              ...
              <div class="text-center mb-8">
                <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-2">{{ plan.nameKey | translate }}</h3>
                <div class="flex items-baseline justify-center">
                  <span class="text-4xl font-bold text-gray-900 dark:text-gray-100">{{ plan.priceKey | translate }}</span>
                  <span class="text-gray-600 dark:text-gray-400 ml-1">{{ 'LANDING.PRICING.PER_MONTH' | translate }}</span>
                </div>
              </div>
              ...
              <ul class="space-y-4 mb-8 flex-grow">
                @for (featureKey of plan.featureKeys; track featureKey) {
                  <li class="flex items-center">
                    <svg class="w-5 h-5 text-[#155347] dark:text-emerald-400 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span class="text-gray-700 dark:text-gray-300">{{ featureKey | translate }}</span>
                  </li>
                }
              </ul>

              <button
                  [class]="'w-full py-3 px-6 rounded-xl font-semibold transition-colors cursor-pointer ' + (plan.popular ? 'bg-[#155347] text-white hover:opacity-90' : 'border-2 border-[#155347] dark:border-emerald-400 text-[#155347] dark:text-emerald-400 hover:bg-[#155347] dark:hover:bg-emerald-400 hover:text-white')"
              >
